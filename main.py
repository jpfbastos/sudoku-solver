...
def solve(img, is_from_camera):

    puzzle, colour = extract_puzzle.read_puzzle(img,
    ...
    if solve_puzzle.solve_sudoku(matrix):
        ...
        if is_from_camera:
            ...
            #cv2.imshow('Solved Sudoku (Press any key to quit)', img)
            cv2.imshow('Solved Sudoku (Press Esc to quit, any other key to solve another puzzle)', img)
        k = cv2.waitKey(0)
        cv2.destroyAllWindows()
        if is_from_camera:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--source', '-src', type=str, default=None, help="Path to the input Sudoku image file (leave "
                                                                      "blank to use camera)")
    ...
    if not input_path:
        while True:
            img_name = get_image_from_camera()
            solve(img_name, True)
    else:
